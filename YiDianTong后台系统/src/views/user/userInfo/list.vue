  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" width="200" align="center">
        <template v-slot="scope">
          <router-link :to="'/user/userInfo/details/'+scope.row.id">
            <el-button type="primary" size="mini" icon="el-icon-document"></el-button>
          </router-link>
...
export default {
  ...
  methods: {
    ...
    changDataStatus(id, status) {
      this.$confirm(`此操作将${status === 1 ? '取消锁定' : '锁定'}该用户账号, 是否继续?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        userInfoApi.updateStatus(id, status)
          .then(response => {
            this.fetchData()
          })
        this.$message({
