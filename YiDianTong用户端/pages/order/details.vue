...
export default {
  ...
  methods: {
    queryPayStatus() {
      weixinApi.queryPayStatus(this.orderId)
        .then(response => {
          if (response.message === "支付中") return
    ...
    pay() {
      ...
      //调用请求
      weixinApi.getQRCode(this.orderId)
        .then(response => {
          this.payObj = response.data
          if (this.payObj.codeUrl == null || this.payObj.codeUrl === '') {
    ...
    init() {
      orderInfoApi.getOrderDetails(this.orderId).then(response => {
