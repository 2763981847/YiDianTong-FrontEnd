...
export default {
  ...
  asyncData({ params, error }) {
    //调用
    return hosp.listHospitalPages(1, 10, null).then((response) => {
      return {
  ...
  methods: {
    ...
    init() {
      //查询医院等级列表
      dict.listByDictCode("Hostype").then((response) => {
        //hostypeList清空
        this.hostypeList = [];
      ...
      //查询地区数据
      dict.listByDictCode("Beijing").then((response) => {
        //districtList清空
        this.districtList = [];
    ...
    listHospitalPages() {
      hosp
        .listHospitalPages(this.page, this.limit, this.searchObj)
        .then((response) => {
    ...
    querySearchAsync(queryString, cb) {
      ...
      hosp.listHospitalsByHosnameLike(queryString).then((response) => {
